...
for (i in categorical_prev_sign_tech_factors) { 
  ...
  pval_chi <- chisq.test(as.factor(RNA_seq_info[,i]), as.factor(day))$p.value
...
for (i in categorical_prev_sign_tech_factors) { 
  ...
  pval_chi <- chisq.test(as.factor(RNA_seq_info[,i]), as.factor(Species))$p.value
...
* Harvest time (BH adjusted p-value = 1.661634e-15)
* Harvest density (BH adjusted p-value = 0.01327875)
* Index sequence (BH adjusted p-value = 5.219213e-15)

The following variables are confounded with species:

* Cell line (BH adjusted p-value = 6.703107e-09)
* SPSX (BH adjusted p-value = 3.402224e-12)
...
plot(as.factor(day), RNA_seq_info[,9], main = "Harvest density versus day (sign.)")
plot(Species, RNA_seq_info[,9], main = "Harvest density versus species (sign.)")
plot(as.factor(day), RNA_seq_info[,10], main = "High confidence purity versus day")
plot(Species, RNA_seq_info[,10], main = "High confidence purity versus species")
plot(as.factor(day), RNA_seq_info[,11], main = "Max. purity versus day")
plot(Species, RNA_seq_info[,11], main = "Max. purity versus species (sign.)")
...
### Cleaner plots of significant technical factors versus our biological variables of interest
